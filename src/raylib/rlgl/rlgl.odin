...
TEXTURE_MIPMAP_BIAS_RATIO            :: 0x4000      // Texture mipmap bias, percentage ratio (custom identifier)
...
VertexBuffer :: struct {
	...
	normals:      [^]f32,                   // Vertex normal (XYZ - 3 components per vertex) (shader-location = 2)
	...
	vboId:        [5]c.uint,                // OpenGL Vertex Buffer Objects id (5 types of vertex data)
}
...
foreign lib {
	...
	PopMatrix           :: proc() ---                                                // Pop latest inserted matrix from stack
	LoadIdentity        :: proc() ---                                                // Reset current matrix to identity matrix
	Translatef          :: proc(x: f32, y: f32, z: f32) ---                          // Multiply the current matrix by a translation matrix
	Rotatef             :: proc(angle: f32, x: f32, y: f32, z: f32) ---              // Multiply the current matrix by a rotation matrix
	Scalef              :: proc(x: f32, y: f32, z: f32) ---                          // Multiply the current matrix by a scaling matrix
	MultMatrixf         :: proc(matf: [^]f32) ---                                    // Multiply the current matrix by another matrix
	Frustum             :: proc(left: f64, right: f64, bottom: f64, top: f64, znear: f64, zfar: f64) ---
	Ortho               :: proc(left: f64, right: f64, bottom: f64, top: f64, znear: f64, zfar: f64) ---
	Viewport            :: proc(x: c.int, y: c.int, width: c.int, height: c.int) --- // Set the viewport area
	SetClipPlanes       :: proc(nearPlane: f64, farPlane: f64) ---                   // Set clip planes distances
	...
	Vertex2i   :: proc(x: c.int, y: c.int) ---             // Define one vertex (position) - 2 int
	Vertex2f   :: proc(x: f32, y: f32) ---                 // Define one vertex (position) - 2 float
	Vertex3f   :: proc(x: f32, y: f32, z: f32) ---         // Define one vertex (position) - 3 float
	TexCoord2f :: proc(x: f32, y: f32) ---                 // Define one vertex (texture coordinate) - 2 float
	Normal3f   :: proc(x: f32, y: f32, z: f32) ---         // Define one vertex (normal) - 3 float
	Color4ub   :: proc(r: u8, g: u8, b: u8, a: u8) ---     // Define one vertex (color) - 4 byte
	Color3f    :: proc(x: f32, y: f32, z: f32) ---         // Define one vertex (color) - 3 float
	Color4f    :: proc(x: f32, y: f32, z: f32, w: f32) --- // Define one vertex (color) - 4 float
	...
	BlitFramebuffer      :: proc(srcX: c.int, srcY: c.int, srcWidth: c.int, srcHeight: c.int, dstX: c.int, dstY: c.int, dstWidth: c.int, dstHeight: c.int, bufferMask: c.int) --- // Blit active framebuffer to main framebuffer
	BindFramebuffer      :: proc(target: c.uint, framebuffer: c.uint) --- // Bind framebuffer (FBO)
	...
	ColorMask               :: proc(r: bool, g: bool, b: bool, a: bool) ---              // Color mask control
	...
	Scissor                 :: proc(x: c.int, y: c.int, width: c.int, height: c.int) --- // Scissor test
	...
	DisableWireMode         :: proc() ---                                                // Disable wire mode
	...
	ClearColor              :: proc(r: u8, g: u8, b: u8, a: u8) ---                      // Clear color buffer with color
	...
	SetBlendFactors         :: proc(glSrcFactor: c.int, glDstFactor: c.int, glEquation: c.int) --- // Set blending mode factor and equation (using OpenGL factors)
	SetBlendFactorsSeparate :: proc(glSrcRGB: c.int, glDstRGB: c.int, glSrcAlpha: c.int, glDstAlpha: c.int, glEqRGB: c.int, glEqAlpha: c.int) --- // Set blending mode factors and equations separately (using OpenGL factors)
	...
	@(link_name="rlglInit")
	Init                 :: proc(width: c.int, height: c.int) --- // Initialize rlgl (buffers, shaders, textures, states)
	@(link_name="rlglClose")
	...
	LoadRenderBatch       :: proc(numBuffers: c.int, bufferElements: c.int) -> RenderBatch --- // Load a render batch system
	...
	LoadVertexBuffer                 :: proc(buffer: rawptr, size: c.int, _dynamic: bool) -> c.uint --- // Load a vertex buffer object
	LoadVertexBufferElement          :: proc(buffer: rawptr, size: c.int, _dynamic: bool) -> c.uint --- // Load vertex buffer elements object
	UpdateVertexBuffer               :: proc(bufferId: c.uint, data: rawptr, dataSize: c.int, offset: c.int) --- // Update vertex buffer object data on GPU buffer
	UpdateVertexBufferElements       :: proc(id: c.uint, data: rawptr, dataSize: c.int, offset: c.int) --- // Update vertex buffer elements data on GPU buffer
	UnloadVertexArray                :: proc(vaoId: c.uint) ---                               // Unload vertex array (vao)
	UnloadVertexBuffer               :: proc(vboId: c.uint) ---                               // Unload vertex buffer object
	SetVertexAttribute               :: proc(index: c.uint, compSize: c.int, type: c.int, normalized: bool, stride: c.int, offset: c.int) --- // Set vertex attribute data configuration
	SetVertexAttributeDivisor        :: proc(index: c.uint, divisor: c.int) ---               // Set vertex attribute data divisor
	SetVertexAttributeDefault        :: proc(locIndex: c.int, value: rawptr, attribType: c.int, count: c.int) --- // Set vertex attribute default value, when attribute to provided
	DrawVertexArray                  :: proc(offset: c.int, count: c.int) ---                 // Draw vertex array (currently active vao)
	DrawVertexArrayElements          :: proc(offset: c.int, count: c.int, buffer: rawptr) --- // Draw vertex array elements
	DrawVertexArrayInstanced         :: proc(offset: c.int, count: c.int, instances: c.int) --- // Draw vertex array (currently active vao) with instancing
	DrawVertexArrayElementsInstanced :: proc(offset: c.int, count: c.int, buffer: rawptr, instances: c.int) --- // Draw vertex array elements with instancing

	// Textures management
	LoadTexture         :: proc(data: rawptr, width: c.int, height: c.int, format: c.int, mipmapCount: c.int) -> c.uint --- // Load texture data
	LoadTextureDepth    :: proc(width: c.int, height: c.int, useRenderBuffer: bool) -> c.uint --- // Load depth texture/renderbuffer (to be attached to fbo)
	LoadTextureCubemap  :: proc(data: rawptr, size: c.int, format: c.int, mipmapCount: c.int) -> c.uint --- // Load texture cubemap data
	UpdateTexture       :: proc(id: c.uint, offsetX: c.int, offsetY: c.int, width: c.int, height: c.int, format: c.int, data: rawptr) --- // Update texture with new data on GPU
	GetGlTextureFormats :: proc(format: c.int, glInternalFormat: ^c.uint, glFormat: ^c.uint, glType: ^c.uint) --- // Get OpenGL internal formats
	...
	GenTextureMipmaps   :: proc(id: c.uint, width: c.int, height: c.int, format: c.int, mipmaps: ^c.int) --- // Generate mipmap data for selected texture
	ReadTexturePixels   :: proc(id: c.uint, width: c.int, height: c.int, format: c.int) -> rawptr --- // Read texture pixel data
	ReadScreenPixels    :: proc(width: c.int, height: c.int) -> [^]u8 --- // Read screen pixel data (color buffer)

	// Framebuffer management (fbo)
	LoadFramebuffer     :: proc() -> c.uint ---         // Load an empty framebuffer
	FramebufferAttach   :: proc(id: c.uint, texId: c.uint, attachType: c.int, texType: c.int, mipLevel: c.int) --- // Attach texture/renderbuffer to a framebuffer
	...
	GetLocationUniform       :: proc(id: c.uint, uniformName: cstring) -> c.int ---     // Get shader location uniform, requires shader program id
	GetLocationAttrib        :: proc(id: c.uint, attribName: cstring) -> c.int ---      // Get shader location attribute, requires shader program id
	...
	SetUniformMatrices       :: proc(locIndex: c.int, mat: [^]Matrix, count: c.int) --- // Set shader value matrices
	...
	ComputeShaderDispatch :: proc(groupX: c.uint, groupY: c.uint, groupZ: c.uint) --- // Dispatch compute shader (equivalent to *draw* for graphics pipeline)
	...
	CopyShaderBuffer    :: proc(destId: c.uint, srcId: c.uint, destOffset: c.uint, srcOffset: c.uint, count: c.uint) --- // Copy SSBO data between buffers
	...
	SetMatrixProjectionStereo :: proc(right: Matrix, left: Matrix) --- // Set eyes projection matrices for stereo rendering
	SetMatrixViewOffsetStereo :: proc(right: Matrix, left: Matrix) --- // Set eyes view offsets matrices for stereo rendering
