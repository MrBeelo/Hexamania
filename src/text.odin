...
DrawText :: proc(text: string, pos: rl.Vector2, size: f32, type := FontType.QUICKSAND_MEDIUM, color := rl.WHITE, border_info := BorderInfo{}, spacing: f32 = 5) {
	...
	if border_info.bordered do for i in -1..=1 do for j in -1..=1 do if i != 0 || j != 0 {
		rl.DrawTextPro(font, str, pos + {f32(i) * border_info.border_thickness, f32(j) * border_info.border_thickness}, 
			0, 0, size, spacing, border_info.border_color)
	}
	
	rl.DrawTextPro(font, str, pos, 0, 0, size, spacing, color)
}
...
DrawTextCenter :: proc(text: string, center: rl.Vector2, size: f32, type := FontType.QUICKSAND_MEDIUM, color := rl.WHITE, border_info := BorderInfo{}, spacing: f32 = 5) {
	...
	if border_info.bordered do for i in -1..=1 do for j in -1..=1 do if i != 0 || j != 0 {
		rl.DrawTextPro(font, str, center + {f32(i) * border_info.border_thickness, f32(j) * border_info.border_thickness}, 
			text_size / 2, 0, size, spacing, border_info.border_color)
	}
	
	rl.DrawTextPro(font, str, center, text_size / 2, 0, size, spacing, color)
}

MeasureText :: proc(text: string, size: f32, type := FontType.QUICKSAND_MEDIUM, spacing: f32 = 5) -> rl.Vector2 {
	str := strings.clone_to_cstring(text)
	return rl.MeasureTextEx(GetFont({type, size}), str, size, spacing)
}