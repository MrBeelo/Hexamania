...
// end include: /tmp/tmp3hcezckg.js
// include: /tmp/tmpstbze5e3.js

    // All the pre-js content up to here must remain later on, we need to run
    // it.
    if ((typeof ENVIRONMENT_IS_WASM_WORKER != 'undefined' && ENVIRONMENT_IS_WASM_WORKER) || (typeof ENVIRONMENT_IS_PTHREAD != 'undefined' && ENVIRONMENT_IS_PTHREAD) || (typeof ENVIRONMENT_IS_AUDIO_WORKLET != 'undefined' && ENVIRONMENT_IS_AUDIO_WORKLET)) Module['preRun'] = [];
    var necessaryPreJSTasks = Module['preRun'].slice();
  // end include: /tmp/tmpstbze5e3.js
// include: /tmp/tmp34cig_dl.js

    if (!Module['preRun']) throw 'Module.preRun should exist because file support used it; did a pre-js delete it?';
    necessaryPreJSTasks.forEach((task) => {
      if (Module['preRun'].indexOf(task) < 0) throw 'All preRun tasks that exist before user pre-js code should remain after; did you replace Module or modify Module.preRun?';
    });
  // end include: /tmp/tmp34cig_dl.js
...
var ASM_CONSTS = {
  148734: () => { if (document.fullscreenElement) return 1; },  
 148780: () => { return Module.canvas.width; },  
 148812: () => { return parseInt(Module.canvas.style.width); },  
 148860: () => { document.exitFullscreen(); },  
 148887: () => { setTimeout(function(){ Module.requestFullscreen(false, false); }, 100); },  
 148959: () => { if (document.fullscreenElement) return 1; },  
 149005: () => { return Module.canvas.width; },  
 149037: () => { return screen.width; },  
 149062: () => { document.exitFullscreen(); },  
 149089: ($0) => { const canvasId = UTF8ToString($0); setTimeout(function() { Module.requestFullscreen(false, true); setTimeout(function() { document.querySelector(canvasId).style.width="unset"; }, 100); }, 100); },  
 149283: () => { return window.innerWidth; },  
 149309: () => { return window.innerHeight; },  
 149336: () => { if (document.fullscreenElement) return 1; },  
 149382: () => { return Module.canvas.width; },  
 149414: () => { return parseInt(Module.canvas.style.width); },  
 149462: () => { if (document.fullscreenElement) return 1; },  
 149508: () => { return Module.canvas.width; },  
 ...
 149664: () => { return Module.canvas.width; },  
 149696: () => { return screen.width; },  
 149721: () => { document.exitFullscreen(); },  
 149748: () => { if (document.fullscreenElement) return 1; },  
 149794: () => { return Module.canvas.width; },  
 149826: () => { return parseInt(Module.canvas.style.width); },  
 149874: () => { document.exitFullscreen(); },  
 149901: ($0) => { Module.canvas.style.opacity = $0; },  
 149939: () => { return screen.width; },  
 149964: () => { return screen.height; },  
 149990: () => { return window.screenX; },  
 150017: () => { return window.screenY; },  
 150044: () => { return window.devicePixelRatio; },  
 150080: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 150133: ($0) => { Module.canvas.style.cursor = UTF8ToString($0); },  
 150184: () => { Module.canvas.style.cursor = 'none'; },  
 150221: ($0, $1, $2, $3) => { try { navigator.getGamepads()[$0].vibrationActuator.playEffect('dual-rumble', { startDelay: 0, duration: $3, weakMagnitude: $1, strongMagnitude: $2 }); } catch (e) { try { navigator.getGamepads()[$0].hapticActuators[0].pulse($2, $3); } catch (e) { } } },  
 150477: ($0) => { Module.canvas.style.cursor = UTF8ToString($0); },  
 150528: () => { if (document.pointerLockElement) return 1; },  
 150575: () => { if (document.fullscreenElement) return 1; },  
 150621: () => { return window.innerWidth; },  
 150647: () => { return window.innerHeight; },  
 150674: ($0, $1, $2, $3, $4) => { if (typeof window === 'undefined' || (window.AudioContext || window.webkitAudioContext) === undefined) { return 0; } if (typeof(window.miniaudio) === 'undefined') { window.miniaudio = { referenceCount: 0 }; window.miniaudio.device_type = {}; window.miniaudio.device_type.playback = $0; window.miniaudio.device_type.capture = $1; window.miniaudio.device_type.duplex = $2; window.miniaudio.device_state = {}; window.miniaudio.device_state.stopped = $3; window.miniaudio.device_state.started = $4; let miniaudio = window.miniaudio; miniaudio.devices = []; miniaudio.track_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == null) { miniaudio.devices[iDevice] = device; return iDevice; } } miniaudio.devices.push(device); return miniaudio.devices.length - 1; }; miniaudio.untrack_device_by_index = function(deviceIndex) { miniaudio.devices[deviceIndex] = null; while (miniaudio.devices.length > 0) { if (miniaudio.devices[miniaudio.devices.length-1] == null) { miniaudio.devices.pop(); } else { break; } } }; miniaudio.untrack_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == device) { return miniaudio.untrack_device_by_index(iDevice); } } }; miniaudio.get_device_by_index = function(deviceIndex) { return miniaudio.devices[deviceIndex]; }; miniaudio.unlock_event_types = (function(){ return ['touchend', 'click']; })(); miniaudio.unlock = function() { for(var i = 0; i < miniaudio.devices.length; ++i) { var device = miniaudio.devices[i]; if (device != null && device.webaudio != null && device.state === miniaudio.device_state.started) { device.webaudio.resume().then(() => { _ma_device__on_notification_unlocked(device.pDevice); }, (error) => {console.error("Failed to resume audiocontext", error); }); } } miniaudio.unlock_event_types.map(function(event_type) { document.removeEventListener(event_type, miniaudio.unlock, true); }); }; miniaudio.unlock_event_types.map(function(event_type) { document.addEventListener(event_type, miniaudio.unlock, true); }); } window.miniaudio.referenceCount += 1; return 1; },  
 152852: () => { if (typeof(window.miniaudio) !== 'undefined') { window.miniaudio.unlock_event_types.map(function(event_type) { document.removeEventListener(event_type, window.miniaudio.unlock, true); }); window.miniaudio.referenceCount -= 1; if (window.miniaudio.referenceCount === 0) { delete window.miniaudio; } } },  
 153156: () => { return (navigator.mediaDevices !== undefined && navigator.mediaDevices.getUserMedia !== undefined); },  
 153260: () => { try { var temp = new (window.AudioContext || window.webkitAudioContext)(); var sampleRate = temp.sampleRate; temp.close(); return sampleRate; } catch(e) { return 0; } },  
 ...
 156308: ($0) => { return window.miniaudio.get_device_by_index($0).webaudio.sampleRate; },  
 156381: ($0) => { var device = window.miniaudio.get_device_by_index($0); if (device.scriptNode !== undefined) { device.scriptNode.onaudioprocess = function(e) {}; device.scriptNode.disconnect(); device.scriptNode = undefined; } if (device.streamNode !== undefined) { device.streamNode.disconnect(); device.streamNode = undefined; } device.webaudio.close(); device.webaudio = undefined; device.pDevice = undefined; },  
 156781: ($0) => { window.miniaudio.untrack_device_by_index($0); },  
 156831: ($0) => { var device = window.miniaudio.get_device_by_index($0); device.webaudio.resume(); device.state = window.miniaudio.device_state.started; },  
 156970: ($0) => { var device = window.miniaudio.get_device_by_index($0); device.webaudio.suspend(); device.state = window.miniaudio.device_state.stopped; }
};
